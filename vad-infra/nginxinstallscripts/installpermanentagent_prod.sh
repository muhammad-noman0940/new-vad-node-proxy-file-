...
cd /home/$USERNAME
#  Install agent
mkdir azagent;cd azagent;curl -fkSL -o vstsagent.tar.gz https://go.microsoft.com/fwlink/?linkid=2066772;tar -zxvf vstsagent.tar.gz; if [ -x "$(command -v systemctl)" ]; then ./config.sh --deploymentgroup --deploymentgroupname "Prod Nginx Servers" --acceptteeeula --deploymentgrouptags "PROD" --adddeploymentgrouptags --agent $HOSTNAME --unattended --url https://devops.dctabudhabi.ae/ --sslskipcertvalidation --work _work --collectionname 'Culture' --projectname 'VADRedesign' --auth PAT --token $PAT --runasservice;  else ./config.sh --deploymentgroup --deploymentgroupname "Prod Nginx Servers" --acceptteeeula --deploymentgrouptags "PROD" --adddeploymentgrouptags --agent $HOSTNAME --url https://devops.dctabudhabi.ae/ --sslskipcertvalidation --work _work --collectionname 'Culture' --projectname 'VADRedesign' --auth PAT --token $PAT; ./run.sh; fi